...
//for my data
    ...
    $my_out = mysqli_fetch_assoc($my_query);
      ...
      $profile_data = "SELECT `id`, `user_id`, `name`, `email`, `gender`, `bio`, `dob`, `address`, `img`, `back_cover` FROM `profile`  WHERE user_id != '".$_SESSION['id']."' order by id desc";
    $profile_query = mysqli_query($conn, $profile_data);

       while($profile_out = mysqli_fetch_assoc($profile_query)){
